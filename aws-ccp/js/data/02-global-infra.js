...
window.CURRICULUM.push(
  {
    ...
    understand: [
      {
        ...
        body:
          ...
          "<p><strong>ではリージョンはどう選ぶか</strong>。公式試験ガイドでも『複数のリージョンを使用するケース』が対象スキルです。判断材料は 4 つ——①<strong>データ主権（法令・規制）</strong>：個人情報を国外に出せない等の要件があるため、<strong>データは原則そのリージョン内に留まる</strong>ことを踏まえて選ぶ。②<strong>利用者への近さ（低レイテンシ）</strong>：日本の利用者向けなら東京。③<strong>使いたいサービスがあるか</strong>：新しいサービスは提供リージョンが限られることがある。④<strong>料金</strong>：リージョンによって単価が異なる。</p>" +
          "<p>さらに<strong>複数リージョンに分けて配置</strong>すれば、地域全体を襲う災害にも耐えられます（<strong>ディザスタリカバリ・事業継続</strong>）。『AZ障害対策＝マルチAZ』『地域まるごとの災害対策・法令対応＝マルチリージョン』と段階で覚えます。</p>",
        diagram:
    ...
    quiz: [
      ...
      {
        q: "国内の法令により、顧客の個人データを日本国内に保管することが求められている。AWS でこの要件に対応するために最も重要な考慮事項はどれか。",
        choices: ["エッジロケーションを増やす", "データを保存するリージョンを日本国内から選ぶ", "インスタンスタイプを大きくする", "サポートプランを上位にする"],
        answer: 1,
        explain: "AWS では<strong>データは原則そのリージョン内に留まる</strong>ため、法令・規制（<strong>データ主権</strong>）への対応はまず<strong>どのリージョンに保存するか</strong>で決まる。リージョン選定の観点は、データ主権・利用者への近さ（低レイテンシ）・使いたいサービスの有無・料金の4つ。",
      },
    ],
  }
);
