...
(function () {
  ...
  function showModule(m) {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = m.title + " | コレダケ学習宅建士";
  }
  ...
  function showCompare() {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = "用語早見表 | コレダケ学習宅建士";
  }
  ...
  function renderCompare() {
    ...
    html += "<h1>🗂 用語早見表</h1>";
    html += "<p>頻出" + rows.length + "用語を一覧。検索とカテゴリで絞り込み、<strong>用語をクリックすると解説ページへ移動</strong>します。下部に「紛らわしいペアの決め手」も。</p></div>";

    html += '<div class="cmp-toolbar"><input type="search" class="cmp-search" id="cmp-search" placeholder="🔍 検索（例: 免許 / 手付 / 開発許可 / 相続）" aria-label="用語を検索"><span class="cmp-count" id="cmp-count"></span></div>';
    ...
    html += '<div class="cmp-wrap"><table class="cmp-table"><thead><tr><th>用語</th><th>役割（一言）</th><th>こう出たら</th><th>要点・ひっかけ</th></tr></thead><tbody id="cmp-body">';
    ...
    rows.forEach(function (r) {
      ...
      html += '<td data-label="こう出たら">' + esc(r.sig) + "</td>";
  ...
  function showContact() {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = "お問い合わせ | コレダケ学習宅建士";
  }
  ...
  function route() {
    ...
    if (hash === "compare") {
      ...
      document.title = "コレダケ学習宅建士｜完全無料で合格する図解学習サイト";
