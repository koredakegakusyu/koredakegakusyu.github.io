...
(function () {
  ...
  function showModule(m) {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = m.title + " | コレダケ学習基本情報";
  }
  ...
  function showCompare() {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = "用語早見表 | コレダケ学習基本情報";
  }
  ...
  function renderCompare() {
    ...
    html += "<h1>🗂 用語早見表</h1>";
    html += "<p>全" + rows.length + "語を一覧。検索とカテゴリで絞り込み、<strong>用語をクリックすると解説ページへ移動</strong>します。下部に「紛らわしいペアの決め手」も。計算公式は左メニューの<strong>🧮 公式早見表</strong>へ。</p></div>";

    html += '<div class="cmp-toolbar"><input type="search" class="cmp-search" id="cmp-search" placeholder="🔍 検索（例: 暗号化 / 稼働率 / DBMS / アジャイル）" aria-label="サービスを検索"><span class="cmp-count" id="cmp-count"></span></div>';
    ...
    html += '<div class="cmp-wrap"><table class="cmp-table"><thead><tr><th>用語</th><th>意味（一言）</th><th>区分・具体例</th><th>要点・ひっかけ</th></tr></thead><tbody id="cmp-body">';
    ...
    rows.forEach(function (r) {
      ...
      html += '<td data-label="用語">' + nameCell + '<span class="cmp-catbadge">' + esc(r.cat) + "</span></td>";
      html += '<td data-label="意味">' + esc(r.role) + "</td>";
      html += '<td data-label="区分">' + esc(r.sig) + "</td>";
  ...
  function showFormulas() {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = "公式早見表 | コレダケ学習基本情報";
  }

  function renderFormulas() {
    var formulas = window.FORMULAS || [];
    var html = '<div class="cmp-head"><div class="crumb"><a href="#home" data-nav>ホーム</a> ／ 公式早見表</div>';
    html += "<h1>🧮 公式早見表</h1>";
    html += "<p>計算問題で使う公式だけを分野別にまとめました。数字を当てはめるだけで解けます。用語の意味は左メニューの<strong>🗂 用語早見表</strong>へ。</p></div>";
    formulas.forEach(function (g) {
      html += '<div class="formula-group"><h3>' + esc(g.cat) + '</h3><div class="formula-list">';
      g.items.forEach(function (it) {
        html += '<div class="formula-card"><div class="formula-name">' + esc(it.name) + "</div>" +
          '<div class="formula-eq">' + esc(it.eq) + "</div>" +
          '<div class="formula-note">' + it.note + "</div></div>";
      });
      html += "</div></div>";
    });
    if (!formulas.length) html += '<p class="cmp-empty">公式データがありません。</p>';
    viewModule.innerHTML = html;
  }
  ...
  function showContact() {
    ...
    window.scrollTo({ top: 0, behavior: "auto" });
    closeSidebar();
    document.title = "お問い合わせ | コレダケ学習基本情報";
  }
  ...
  function renderContact() {
    ...
    html += '<div class="formula-card"><div class="formula-name">メールでのお問い合わせ</div>' +
      ...
      '<div class="formula-note">上記アドレス宛にメールをお送りください。内容確認のうえ対応いたします（返信をお約束するものではありません）。</div></div>';
    html += "</div></div>";
    viewModule.innerHTML = html;
  }
  ...
  function route() {
    ...
    if (hash === "compare") {
      ...
      document.title = "コレダケ学習基本情報｜完全無料で合格する図解学習サイト";
